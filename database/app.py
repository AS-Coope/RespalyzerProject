...
def record():
    try:
        ...
        cursor.execute(f"INSERT INTO public.recordings (recording, reading, date_recorded, user_id, disease_id, likelihood) VALUES ('{file.filename}','{outcome}', NOW(), (SELECT MAX(user_id) FROM \"user\"), {largest_index}, {likelihood})")
        cnx.commit()
        cursor.close()
        cnx.close()
